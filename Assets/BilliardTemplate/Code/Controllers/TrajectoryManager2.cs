...
namespace ibc.controller
{
    ...
    public class TrajectoryManager2 : MonoBehaviour
    {
        // (省略 - 既存の変数はそのまま)
        ...
        // 軌道計算と表示の共通ロジック (新規関数)
        public void CalculateAndShowTrajectories()
        {
            if (_showTrajectories)
            {
                ShowTrajectoriesInternal();
                CalculateAndShowTrajectoriesInternal();
            }
        }

        private void CalculateAndShowTrajectoriesInternal()
        {
            // ① 既存の初期化まわりはそのまま
            ResetTrajectories();
            ResetBalls();
