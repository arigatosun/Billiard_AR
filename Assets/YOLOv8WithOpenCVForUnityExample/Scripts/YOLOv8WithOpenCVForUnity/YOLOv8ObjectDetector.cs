    ...
    public class YOLOv8ObjectDetector
    {
        ...
        public virtual void visualize(Mat image, Mat results, bool print_results = false, bool isRGB = false)
        {
            ...
            foreach (var d in data.Reverse())
            {
                ...
                // 半径を計算（幅と高さの大きい方を使用）
                int radius = (int)(Math.Max(right - left, bottom - top) / 2 * 2f);

                Scalar c = palette[classId % palette.Count];
                Scalar color = isRGB ? c : new Scalar(c.val[2], c.val[1], c.val[0], c.val[3]);

                //四角の枠を描画
                //Imgproc.rectangle(image, new Point(left, top), new Point(right, bottom), color, 5);

                //円の枠を描画
                Imgproc.circle(image, center, radius, color, 5);
                ...
                // ラベルの位置を円の上部に設定
                float labelY = (float)(center.y - radius - 10); // 円の上部から10ピクセル上
                float labelX = (float)(center.x - labelSize.width / 2); // 中央揃え
